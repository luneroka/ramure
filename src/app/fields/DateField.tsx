...
function kindLabel(lang: Lang, k: DateKind): string {
  const fr: Record<string, string> = { exact: 'Le', about: 'Vers', estimated: 'Estimé', calculated: 'Calculé', before: 'Avant', after: 'Après', between: 'Entre', 'from-to': 'De … à', from: 'À partir de', to: 'Jusqu’à' };
  const en: Record<string, string> = { exact: 'On', about: 'About', estimated: 'Estimated', calculated: 'Calculated', before: 'Before', after: 'After', between: 'Between', 'from-to': 'From … to', from: 'From', to: 'Until' };
...
interface Triplet { day: string; month: string; year: string }
const toTriplet = (x?: SimpleDate): Triplet => ({ day: x?.day ? String(x.day) : '', month: x?.month ? String(x.month) : '', year: x?.year !== undefined ? String(x.year) : '' });
...
export function DateField({ value, onChange, lang, label }: Props) {
  ...
  const [kind, setKind] = useState<DateKind>(value?.kind && KINDS.includes(value.kind) ? value.kind : value?.kind === 'calculated' ? 'estimated' : 'exact');
  ...
  const emit = (k: DateKind, ta: Triplet, tb: Triplet) => {
    ...
    if (!d1) { onChange(undefined); return; }
  ...
  const triplet = (tr: Triplet, set: (t: Triplet) => void, which: 'a' | 'b') => (
    <span className="date-triplet">
      <input inputMode="numeric" pattern="[0-9]*" placeholder={lang === 'fr' ? 'JJ' : 'DD'} aria-label={lang === 'fr' ? 'Jour' : 'Day'} value={tr.day} maxLength={2}
        onChange={(e) => { const n = { ...tr, day: e.target.value.replace(/\D/g, '') }; set(n); emit(kind, which === 'a' ? n : a, which === 'b' ? n : b); }} />
      <select aria-label={lang === 'fr' ? 'Mois' : 'Month'} value={tr.month} onChange={(e) => { const n = { ...tr, month: e.target.value }; set(n); emit(kind, which === 'a' ? n : a, which === 'b' ? n : b); }}>
        <option value="">{lang === 'fr' ? 'mois' : 'month'}</option>
        {months.map((m, i) => <option key={m} value={String(i + 1)}>{m}</option>)}
      </select>
      <input inputMode="numeric" pattern="[0-9]*" placeholder={lang === 'fr' ? 'AAAA' : 'YYYY'} aria-label={lang === 'fr' ? 'Année' : 'Year'} value={tr.year} maxLength={4} className="year"
        onChange={(e) => { const n = { ...tr, year: e.target.value.replace(/\D/g, '') }; set(n); emit(kind, which === 'a' ? n : a, which === 'b' ? n : b); }} />
    </span>
  );
    ...
    <div className="date-field">
      <div className="date-head">
        ...
        <button type="button" className="link small" onClick={() => {
          if (mode === 'guided') { setText(value ? formatDate(value, lang) : ''); setMode('text'); }
          else if (isGuidable(value)) { setKind(value?.kind && KINDS.includes(value.kind) ? value.kind : 'exact'); setA(toTriplet(value?.date)); setB(toTriplet(value?.date2)); setMode('guided'); }
        }} disabled={mode === 'text' && !isGuidable(value)} title={mode === 'text' && !isGuidable(value) ? t(lang, 'dateNotGuidable') : undefined}>
          {mode === 'guided' ? t(lang, 'dateFreeText') : t(lang, 'dateGuided')}
        </button>
      </div>
      {mode === 'guided' ? (
        <div className="date-guided">
          <select aria-label={t(lang, 'dateKind')} value={kind} onChange={(e) => { const k = e.target.value as DateKind; setKind(k); emit(k, a, b); }}>
            {KINDS.map((k) => <option key={k} value={k}>{kindLabel(lang, k)}</option>)}
          </select>
          {triplet(a, setA, 'a')}
      ...
      <span className={`date-preview ${value?.kind === 'phrase' ? 'warn' : ''}`}>{value ? (value.kind === 'phrase' ? t(lang, 'dateUnreadable') : formatDate(value, lang)) : ''}</span>
    </div>
  );
}
