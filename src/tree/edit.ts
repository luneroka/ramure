...
export function nextId(tree: Tree, prefix: 'I' | 'F' | 'S' | 'R' | 'M'): string {
  const table = prefix === 'I' ? tree.individuals : prefix === 'F' ? tree.families : prefix === 'S' ? tree.sources : prefix === 'R' ? tree.repositories : tree.media;
  let max = 0;
...
export function createPerson(tree: Tree, data: NewPerson = {}): EditResult {
  const id = nextId(tree, 'I');
...
export function deletePerson(tree: Tree, id: string): EditResult {
  ...
  const nextFocus = ind.partnerIn.map((f) => tree.families[f]).flatMap((f) => [f?.husbandId, f?.wifeId]).find((p) => p && p !== id && t.individuals[p])
    ?? ind.childOf.map((l) => tree.families[l.familyId]).flatMap((f) => [f?.husbandId, f?.wifeId]).find((p) => p && t.individuals[p])
...
function birthFamily(tree: Tree, childId: string): { tree: Tree; fam: Family } {
  ...
  const fam = newFamily(nextId(tree, 'F'));
 ...
 */
export function addParent(tree: Tree, childId: string, slot: 'father' | 'mother', data: NewPerson = {}): EditResult {
  const { tree: t0, fam } = birthFamily(tree, childId);
  ...
  const created = createPerson(t0, { ...data, surname, sex: data.sex ?? (slot === 'father' ? 'M' : 'F') });
...
export function addPartner(tree: Tree, personId: string, data: NewPerson = {}): EditResult {
  const person = must(tree, personId);
  const sex = data.sex ?? (person.sex === 'M' ? 'F' : person.sex === 'F' ? 'M' : 'U');
  const created = createPerson(tree, { ...data, sex });
  const pid = created.focusId!;
  let t = created.tree;
  const fam = newFamily(nextId(t, 'F'));
 ...
 */
export function addChild(tree: Tree, personId: string, data: NewPerson = {}, familyId?: string): EditResult {
  const person = must(tree, personId);
  let t = tree;
  let fam: Family | undefined = familyId ? t.families[familyId] : person.partnerIn.length === 1 ? t.families[person.partnerIn[0]!] : undefined;
  if (!fam) {
    if (person.partnerIn.length > 1 && !familyId) throw new Error('choose a family');
    fam = newFamily(nextId(t, 'F'));
    if (person.sex === 'F') fam.wifeId = personId; else fam.husbandId = personId;
    t = withFamily(t, fam);
  ...
  const created = createPerson(t, { ...data, surname });
...
export function addSibling(tree: Tree, personId: string, data: NewPerson = {}): EditResult {
  const { tree: t0, fam } = birthFamily(tree, personId);
  const person = must(t0, personId);
  const surname = data.surname ?? person.names[0]?.surname ?? '';
  const created = createPerson(t0, { ...data, surname });
  ...
  const ids = [...f.childIds, sid];
  const y = year(sid);
  if (y !== undefined) ids.sort((a, b) => (year(a) ?? Infinity) - (year(b) ?? Infinity));
  t = withFamily(t, { ...f, childIds: ids });
...
export function linkPartner(tree: Tree, personId: string, partnerId: string): EditResult {
  if (personId === partnerId) throw new Error('same person');
  const person = must(tree, personId), partner = must(tree, partnerId);
...
export function mergePeople(tree: Tree, keepId: string, dropId: string): EditResult {
  ...
  const keep = must(tree, keepId), drop = must(tree, dropId);
  ...
  for (const link of drop.childOf) {
    ...
    t = withFamily(t, { ...fam, childIds: fam.childIds.map((c) => (c === dropId ? keepId : c)).filter((c, i, arr) => arr.indexOf(c) === i) });
  }
  for (const fid of drop.partnerIn) {
    const fam = t.families[fid];
    if (!fam) continue;
    if (!merged.partnerIn.includes(fid)) merged.partnerIn.push(fid);
    t = withFamily(t, { ...fam, husbandId: fam.husbandId === dropId ? keepId : fam.husbandId, wifeId: fam.wifeId === dropId ? keepId : fam.wifeId });
  }
  ...
  for (let i = 0; i < fams.length; i++) {
    for (let j = i + 1; j < fams.length; j++) {
      const a = t.families[fams[i]!.id], b = t.families[fams[j]!.id];
      ...
      for (const cid of b.childIds) {
        ...
        if (c) t = withIndividual(t, { ...c, childOf: c.childOf.map((l) => (l.familyId === b.id ? { ...l, familyId: a.id } : l)).filter((l, k, arr) => arr.findIndex((x) => x.familyId === l.familyId) === k) });
      }
    }
  }
...
export function blankEvent(type: EventType, tag?: string): Event {
  const tags: Record<string, string> = { birth: 'BIRT', baptism: 'CHR', death: 'DEAT', burial: 'BURI', marriage: 'MARR', divorce: 'DIV', occupation: 'OCCU', residence: 'RESI', census: 'CENS', custom: 'EVEN', cremation: 'CREM', adoption: 'ADOP', education: 'EDUC', religion: 'RELI', retirement: 'RETI', emigration: 'EMIG', immigration: 'IMMI', naturalization: 'NATU', probate: 'PROB', will: 'WILL', graduation: 'GRAD', confirmation: 'CONF', 'first-communion': 'FCOM', title: 'TITL', description: 'DSCR', engagement: 'ENGA', 'marriage-banns': 'MARB', annulment: 'ANUL', separation: 'EVEN' };
  return newEvent(type, tag ?? tags[type] ?? 'EVEN');
}

/** Build an empty tree with a single person, for starting from scratch. */
export function newTree(given: string, surname: string, sex: Sex): EditResult {
  const tree: Tree = { header: { notes: [], language: 'French' }, individuals: {}, families: {}, sources: {}, repositories: {}, media: {}, extra: [], importNotes: [] };
  return createPerson(tree, { given, surname, sex });
}
