...
function components(tree: Tree): Comp[] {
  ...
  for (const start of Object.keys(tree.individuals)) {
    ...
    while (queue.length) {
      ...
      for (const l of ind.childOf) { const f = tree.families[l.familyId]; if (f) for (const p of [f.husbandId, f.wifeId]) if (p) visit(p, g + 1); }
...
interface Placed { id: string; gen: number; x: number; order: number }
...
export function layoutEverything(tree: Tree, opts: LayoutOptions = DEFAULT_LAYOUT): Layout {
  ...
  for (const comp of compData) {
    ...
    for (const p of placed) { minX = Math.min(minX, p.x); maxX = Math.max(maxX, p.x + opts.cardW); }
    const dx = offsetX - minX;
    const byId = new Map<string, LayoutNode>();
    for (const p of placed) {
      const n: LayoutNode = { id: p.id, x: p.x + dx, y: -p.gen * rowH - opts.cardH / 2, w: opts.cardW, h: opts.cardH, gen: p.gen, dup: 0, role: 'descendant' };
      nodes.push(n);
      byId.set(p.id, n);
      allGens.add(p.gen);
    }
    ...
    for (const fid of new Set(comp.ids.flatMap((id) => tree.individuals[id]!.partnerIn))) {
      ...
      if (h && w) {
        ...
        const y = l.y + l.h / 2;
        links.push({ kind: 'partner', points: [[l.x + l.w, y], [r.x, y]] });
        ax = (l.x + l.w + r.x) / 2; ay = y;
        if (r.x - (l.x + l.w) > opts.partnerGap * 3) { ax = l.x + l.w / 2; ay = l.y + l.h; }
      } else {
        const p = h ?? w;
        if (!p) continue;
        ax = p.x + p.w / 2; ay = p.y + p.h;
      }
      const kids = f.childIds.map((c) => byId.get(c)).filter((n): n is LayoutNode => !!n);
      if (!kids.length) continue;
      const busY = (h ?? w)!.y + opts.cardH + opts.rowGap / 2;
      links.push({ kind: 'child', points: [[ax, ay], [ax, busY]] });
      const centers = kids.map((k) => k.x + k.w / 2);
      for (const k of kids) links.push({ kind: 'child', points: [[k.x + k.w / 2, busY], [k.x + k.w / 2, k.y]] });
      links.push({ kind: 'child', points: [[Math.min(ax, ...centers), busY], [Math.max(ax, ...centers), busY]] });
    }
  ...
  for (const g of rows) labels.set(g, `G${globalMaxGen - g + 1}`);
...
function layoutComponent(tree: Tree, ids: string[], gen: Map<string, number>, opts: LayoutOptions): Placed[] {
  ...
  const partnersOf = (id: string): string[] => tree.individuals[id]!.partnerIn.map(fam).flatMap((f) => (f ? [f.husbandId, f.wifeId] : [])).filter((p): p is string => !!p && p !== id && idSet.has(p));
  const childrenOf = (id: string): string[] => tree.individuals[id]!.partnerIn.map(fam).flatMap((f) => f?.childIds ?? []).filter((c) => idSet.has(c));
  const parentsOf = (id: string): string[] => tree.individuals[id]!.childOf.map((l) => fam(l.familyId)).flatMap((f) => (f ? [f.husbandId, f.wifeId] : [])).filter((p): p is string => !!p && idSet.has(p));

  // ----- Initial order: depth-first walk of descendants from the oldest people, families kept together.
  const rows = new Map<number, string[]>();
  const placed = new Set<string>();
  const push = (id: string) => { if (placed.has(id)) return; placed.add(id); const g = gen.get(id)!; if (!rows.has(g)) rows.set(g, []); rows.get(g)!.push(id); };
  ...
  const reindex = () => { for (const [, row] of rows) row.forEach((id, i) => index.set(id, i)); };
  const bary = (id: string): number | undefined => {
    const neigh = [...parentsOf(id), ...childrenOf(id), ...partnersOf(id)].map((n) => index.get(n)).filter((v): v is number => v !== undefined);
  ...
  const keepCouplesTogether = (row: string[]) => {
    ...
    for (const id of row) {
      ...
      for (const p of partnersOf(id)) if (!done.has(p) && gen.get(p) === gen.get(id)) { out.push(p); done.add(p); }
    }
    return out;
  };
  reindex();
  ...
  for (const g of gens) {
    const row = rows.get(g)!;
    let cx = 0;
    row.forEach((id, i) => { x.set(id, cx); cx += opts.cardW + gapAfter(row, i); });
  }
  ...
  const placeRow = (row: string[], want: Map<string, number>) => {
    interface Block { ids: string[]; offsets: number[]; width: number; want?: number }
    const blocks: Block[] = [];
    let i = 0;
    while (i < row.length) {
      const ids = [row[i]!];
      let j = i;
      while (j + 1 < row.length && partnersOf(row[j]!).includes(row[j + 1]!)) { ids.push(row[j + 1]!); j++; }
      const offsets = ids.map((_, k) => k * (opts.cardW + opts.partnerGap));
      const wants = ids.map((id, k) => { const w = want.get(id); return w === undefined ? undefined : w - offsets[k]!; }).filter((v): v is number => v !== undefined);
      const current = x.get(ids[0]!)!;
      blocks.push({ ids, offsets, width: offsets[offsets.length - 1]! + opts.cardW, want: wants.length ? wants.reduce((a, b) => a + b, 0) / wants.length : current });
      i = j + 1;
    }
    const ltr: number[] = [];
    let minX = -Infinity;
    for (const b of blocks) { const v = Math.max(b.want!, minX); ltr.push(v); minX = v + b.width + opts.siblingGap; }
    const rtl: number[] = new Array(blocks.length);
    let maxX = Infinity;
    for (let k = blocks.length - 1; k >= 0; k--) { const b = blocks[k]!; const v = Math.min(b.want!, maxX); rtl[k] = v; maxX = v - opts.siblingGap - (k > 0 ? blocks[k - 1]!.width : 0); }
    let shift = 0;
  ...
  for (let iter = 0; iter < 10; iter++) {
    ...
    for (const g of [...gens].reverse()) {
      ...
      for (const id of row) { const d = desired(id, iter > 2, true); if (d !== undefined) want.set(id, d); }
      placeRow(row, want);
    }
    ...
    for (const g of gens) {
      const row = rows.get(g)!;
      const want = new Map<string, number>();
      for (const id of row) { const d = desired(id, true, iter > 2); if (d !== undefined) want.set(id, d); }
      placeRow(row, want);
    }
  }
